 ...
 *   修改日期：2021年11月28日 星期日 13时11分38秒
 *   描    述：
 *
 *================================================================*/
#include "channels.h"
...
static void handle_power_module_group_info(channel_info_t *channel_info, uint16_t module_require_voltage, uint16_t module_group_require_current)
{
	...
	list_for_each_entry(power_module_group_info, head, power_module_group_info_t, list) {
		...
		module_power_limit_correction(channels_settings, &module_require_voltage, &module_require_current);

		for(i = 1; i <= power_module_number_per_power_module_group; i++) {
			uint16_t module_require_current_try = module_group_require_current / i;
			output_modules = i;

			if((module_require_current_try == 0) && (channel_info->status.require_output_current != 0)) {
				module_require_current_try = channels_settings->module_min_output_current;
				//debug("module_require_current_try:%d", module_require_current_try);
			}

			module_require_current = module_require_current_try;

			module_voltage_current_correction(channels_settings, &module_require_voltage, &module_require_current);
			module_power_limit_correction(channels_settings, &module_require_voltage, &module_require_current);

			if(module_require_current == module_require_current_try) {
				break;
			}
		}
		...
		list_for_each_entry(power_module_item_info, head1, power_module_item_info_t, list) {
			if(output_modules > 0) {
				...
				power_module_item_info->status.require_output_voltage = 0;
				power_module_item_info->status.require_output_current = 0;
				//debug("setting module_id:%d, require_output_voltage:%d, require_output_current:%d",
				//      power_module_item_info->module_id,
				//      power_module_item_info->status.require_output_voltage,
				//      power_module_item_info->status.require_output_current);
